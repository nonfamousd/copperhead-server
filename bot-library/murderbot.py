...
class RobotPlayer:
    ...
    def calculate_move(self) -> str | None:
        ...
        # Find the nearest food (for opportunistic grabs). We will consider
        # food only if it's within a distance threshold that depends on
        # aggression (difficulty).
        nearest_food = None
        nearest_dist = float('inf')
        for food in foods:
            dist = abs(head[0] - food["x"]) + abs(head[1] - food["y"])
            # Slight preference for apples when equally distant
        ...
        # Identify opponent and their likely next position (one step ahead)
        ...
        if opponent and opponent.get("body"):
            ...
            dir_vec = {"up": (0, -1), "down": (0, 1), "left": (-1, 0), "right": (1, 0)}.get(opp_dir, (-1, 0))
        ...
        for move in safe_moves:
            ...
            # Prefer staying away from edges to keep maneuverability
            ...
            # Random factor decreases with difficulty
